...
public static class GuarantorEndpoints
{
    public static void MapGuarantorEndpoints(this WebApplication app)
    {
        ...
        group.MapPost("/guarantors/{guarantorId:guid}/submit",
            async (Guid guarantorId, [FromBody] SubmitGuarantorDataDto dto, IGuarantorService svc, ClaimsPrincipal user) =>
        {
            if (!TryGetUserId(user, out var userId)) return Results.Unauthorized();
            try { return Results.Ok(await svc.SubmitDataAsync(guarantorId, userId, dto)); }
            catch (KeyNotFoundException e) { return Results.NotFound(e.Message); }
            catch (UnauthorizedAccessException) { return Results.Forbid(); }
            catch (InvalidOperationException e) { return Results.BadRequest(new { error = e.Message }); }
        });
        ...
        guest.MapPost("/{token}/submit", async (string token, [FromBody] SubmitGuarantorDataDto dto, IGuarantorService svc) =>
        {
            try { return Results.Ok(await svc.SubmitDataByTokenAsync(token, dto)); }
            catch (UnauthorizedAccessException) { return Results.Unauthorized(); }
            catch (InvalidOperationException e) { return Results.BadRequest(new { error = e.Message }); }
        });
