...
public static class LeaseEndpoints
{
    public static void MapLeaseEndpoints(this WebApplication app)
    {
        ...
        // POST /api/leases/{leaseId}/simulate-signature-complete  (Development-only, Mock provider)
        // QA/E2E helper: drives the SAME completion path as the signing webhook
        // (SigningProviderService.HandleSignerCompletedAsync) with no HMAC and no external calls.
        // Completes the next unsigned signer (respecting signer order) unless ?signer= targets a
        // specific one (role name or user id). Uses the lease's existing contract PDF as the
        // "signed" artifact. Returns 404 outside Development/staging-simulations.
        group.MapPost("/{leaseId:guid}/simulate-signature-complete",
            async (Guid leaseId, string? signer,
                   LeasingDbContext db,
                   ISigningProviderService signingSvc,
                   ...
                   ClaimsPrincipal user) =>
            {
                ...
                if (!string.Equals(configuration["ElectronicSignature:Provider"], "Mock", StringComparison.OrdinalIgnoreCase))
                    return Results.BadRequest(new { error = "A simulação de assinatura apenas está disponível com ElectronicSignature:Provider=Mock." });
                ...
                try
                {
                    ...
                    if (!string.IsNullOrEmpty(lease.SignatureProvider)
                        && !string.Equals(lease.SignatureProvider, "Mock", StringComparison.OrdinalIgnoreCase))
                        return Results.BadRequest(new { error = "Este arrendamento usa um fornecedor de assinatura real; a simulação não se aplica." });

                    if (lease.Signatures.Count == 0 || lease.Signatures.All(s => s.Signed))
                        return Results.BadRequest(new { error = "Não há signatários pendentes para simular." });

                    var next = lease.Signatures
                        .Where(s => !s.Signed)
                    ...
                    {
                        target = ResolveSimulatedSigner(lease, signer!);
                        if (target.SequenceOrder != next.SequenceOrder)
                            return Results.BadRequest(new { error = $"Assinatura fora de ordem: o próximo signatário é {next.Role}. Para simular use signer={next.Role}." });
                    }

                    // If no external request exists yet, initiate one through the normal embedded
                    // signing path. This sets SignatureProvider="Mock", ExternalSigningRequestId,
                    // each signer's ExternalSignerId and stores the contract PDF inside the mock.
                    if (string.IsNullOrEmpty(lease.ExternalSigningRequestId))
                    {
                        await signingSvc.InitiateEmbeddedSigningAsync(leaseId, target.UserId);
                        lease = await db.Leases
                            .Include(l => l.Signatures)
                            .FirstOrDefaultAsync(l => l.Id == leaseId)
                            ?? throw new KeyNotFoundException("Arrendamento não encontrado.");
                        target = lease.Signatures.Single(s => s.UserId == target.UserId && !s.Signed);
                    }

                    var signerEmail = (await userService.GetProfileDtoAsync(target.UserId))?.Email;
                    await signingSvc.HandleSignerCompletedAsync(
                        lease.ExternalSigningRequestId!,
                        signerEmail ?? $"{target.UserId}@trustrent.local",
                        target.ExternalSignerId);
                    ...
                    {
                        leaseId = lease.Id,
                        completedSigner = target.Role.ToString(),
                        completedSignerUserId = target.UserId,
                        leaseStatus = lease.Status.ToString(),
                        signedCount = lease.Signatures.Count(s => s.Signed),
                        requiredSignaturesCount = lease.Signatures.Count,
                        allSigned = lease.Signatures.All(s => s.Signed)
                    });
                }
                catch (KeyNotFoundException e) { return Results.NotFound(e.Message); }
                catch (UnauthorizedAccessException) { return Results.Forbid(); }
    ...
    private static LeaseSignature ResolveSimulatedSigner(Lease lease, string signer)
    {
        if (Guid.TryParse(signer, out var signerUserId))
        {
            return lease.Signatures.FirstOrDefault(s => s.UserId == signerUserId)
                ?? throw new KeyNotFoundException("Signatário não encontrado neste arrendamento.");
        }

        var byRole = lease.Signatures.FirstOrDefault(s =>
            string.Equals(s.Role.ToString(), signer, StringComparison.OrdinalIgnoreCase));
        if (byRole != null) return byRole;

        throw new KeyNotFoundException($"Signatário desconhecido: {signer}. Use um role (Landlord/Tenant/CoTenant/Guarantor) ou um UserId.");
    }
